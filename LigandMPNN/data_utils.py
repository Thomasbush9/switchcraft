...
def parse_PDB(
    ...
    if other_atoms is not None:     # extend to other erroneously parsed atoms if needed
        names = other_atoms.getNames()
        elems = other_atoms.getElements()
        ...
        for n, e in zip(names, elems):
            ...
            if n.startswith("CA") and e == "A":
                fixed.append("CA")
            else:
                fixed.append(e)

        other_atoms.setElements(fixed)
